...
def make_train(config):
    ...
    def train(entropy_coeff, reg_alpha, param_alpha, learning_rate, rng):
        ...
        def alpha_softargmax(scores):
            return jax.vmap(partial(param_divergence.softargmax, prior=None, beta=1.0), in_axes=0)(
        ...
        if config["ANNEAL_LR"]:
            ...
            tx = optax.chain(optax.clip_by_global_norm(config["MAX_GRAD_NORM"]), optax.adam(learning_rate, eps=1e-5))
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        uniform_distribution = jnp.ones_like(pi.probs) / pi.probs.shape[-1]
                        entropy = reg_divergence.divergence(pi.probs, uniform_distribution).mean()
                        #pi.entropy().mean()
...
def run():
    ...
    with open("deepsea_results_10.pkl", "wb") as fp:
